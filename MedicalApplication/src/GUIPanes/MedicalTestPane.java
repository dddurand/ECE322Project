...
public class MedicalTestPane extends JPanel implements ActionListener {
	...
	public MedicalTestPane(Database database) {
		...
		back_test.setFont(MainWindow.FONT);
	...
	private void renderSearch() {
		...
		Record_id.setFont(MainWindow.FONT);
		add(Record_id);
		add(record_id);
	...
	private void renderResults() {
		...
		searchResults = database.TestRecordSearch(PatientID, DoctorID, TestID, recordID);

		if (searchResults != null) {

			for (int i = 0; i < searchResults.size(); i++) {

				ptable.addRow(new Object[] { searchResults.elementAt(i).getPatientInfo(),
	...
	private void renderMedicalTest() {
		...
		test_lab.setFont(MainWindow.FONT);
		add(test_lab);
		add(lab);
		...
		try {
			...
			lab.setSelectedIndex(-1);
		}

		/* Result */
		JLabel test_result = new JLabel("Result");
		test_result.setFont(MainWindow.FONT);
		add(test_result);
		add(test_results);
		...
		test_date.setFont(MainWindow.FONT);
		add(test_date);
		add(date);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == search) {
			...
			if (results.getSelectedRow() != -1) {
				...
				JOptionPane.showMessageDialog(this, "No Result Selected", "Failure",
			...
			if (!MainWindow.DEBUG) {
				...
				try {
					...
					try {
						...
						JOptionPane.showMessageDialog(this, "Invalid/No Lab Specified", "Failure",
								JOptionPane.ERROR_MESSAGE);
						return;
					}
